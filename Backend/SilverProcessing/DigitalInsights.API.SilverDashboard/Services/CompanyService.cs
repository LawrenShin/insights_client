            ...
            else
            {
                ...
                foreach (var property in properties.Values)
                {
                    var result = (property.PropertyName.ToLower() switch
                    {
                        ...
                        "genderratioexecutive" => ValidationHelper.ValidateAndSetProperty(property, () => metricSource.GenderRatioExecutive, x => target.GenderRatioExecutive = x),
        ...
        private void FillCompanyIndustries(CompanyDTO source, Company targetCompany)
        {
            ...
            foreach (var companyIndustry in source.Industries)
            {
                ...
                foreach(var property in properties.Values)
                {
                    ...
                    {
                        "industry" => true,
                        "industrycodetype" => ValidationHelper.ValidateAndSetProperty(property, () => companyIndustry.IndustryCodeType, x => targetEntity.IndustryCodeType = (DB.Common.Enums.IndustryCode)x.Value,
                            Enum.GetValues<DB.Common.Enums.IndustryCode>().Select(x=>(int)x).ToHashSet()),
                        "industrycode" => ValidationHelper.ValidateAndSetProperty(property, () => companyIndustry.IndustryCode, x => targetEntity.IndustryCode = x.Value),
